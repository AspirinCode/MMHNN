...
from models.gnn_new import GINet
...
class Model001(nn.Module):
    def __init__(self, num_layer=5, emb_dim=300, feat_dim=256, dropout_gin=0, pool='mean', device=torch.device("cuda:0"),
        ...
        self.fc2 = nn.Linear(128, 64)
        self.bn2 = nn.BatchNorm1d(64)  # 添加批归一化层
        self.dropout2 = nn.Dropout(p=0.5)  # 添加Dropout层
        self.fc3 = nn.Linear(64, 1)

        # self.fc1_gib = nn.Linear(emb_dim*2, 128)
        # self.fc2_gib = nn.Linear(128,64)
        # self.fc3_gib = nn.Linear(64, 1)
        self.fc1_gib = nn.Linear(emb_dim*2, 128)
        self.bn1_gib = nn.BatchNorm1d(128)
        self.dropout1_gib = nn.Dropout(p=0.5)
        self.fc2_gib = nn.Linear(128, 64)
        self.bn2_gib = nn.BatchNorm1d(64)
        self.dropout2_gib = nn.Dropout(p=0.5)
        self.fc3_gib = nn.Linear(64, 1)
    ...
    def forward(self,batch,device):
        ...
        # 先创建 graph_1 和 graph_2 的副本
        graph_1_clone = graph_1.clone()
        graph_2_clone = graph_2.clone()

        h_global_1, h_global_2 = self.gin(graph_1, graph_2)
        h_global = torch.cat((h_global_1, h_global_2), dim=1)
        h_global = F.relu(self.bn1(self.fc1(h_global)))
        h_global = self.dropout1(h_global)
        h_global = F.relu(self.bn2(self.fc2(h_global)))
        h_global = self.dropout2(h_global)
        x = self.fc3(h_global)

        # h_global_1_gib,h_global_2_gib,KL_loss=self.gin(graph_1_clone,graph_2_clone,gib=True)
        # h_global_gib = torch.cat((h_global_1_gib,h_global_2_gib), dim=1)
        # h_global_gib = F.relu(self.fc1(h_global_gib))
        # h_global_gib = F.relu(self.fc2(h_global_gib))
        # x_gib = self.fc3(h_global_gib)
        h_global_1_gib, h_global_2_gib, KL_loss = self.gin(graph_1_clone, graph_2_clone, gib=True)
        h_global_gib = torch.cat((h_global_1_gib, h_global_2_gib), dim=1)
        h_global_gib = F.relu(self.bn1_gib(self.fc1_gib(h_global_gib)))
        h_global_gib = self.dropout1_gib(h_global_gib)
        h_global_gib = F.relu(self.bn2_gib(self.fc2_gib(h_global_gib)))
        h_global_gib = self.dropout2_gib(h_global_gib)
        x_gib = self.fc3_gib(h_global_gib)

        return x,x_gib,KL_loss
    def pretrain_result(self, data):
        h_global, out_global, h_sub, out_sub=self.gin(data)
        return out_global,out_sub
    def get_subgraph(self,batch,device):
        graph_1,  graph_2, label = batch
        graph_1 = graph_1.to(device)
        graph_2 = graph_2.to(device)
        # 先创建 graph_1 和 graph_2 的副本
        ...
        lambda_pos_1_list,lambda_pos_2_list,zitu_1_list,zitu_2_list,data_1_motif_batch,data_2_motif_batch = self.gin.get_subgraph(graph_1_clone, graph_2_clone, gib=True)
        return  lambda_pos_1_list,lambda_pos_2_list,zitu_1_list,zitu_2_list,data_1_motif_batch,data_2_motif_batch,graph_1.x,graph_2.x,graph_1.motif_num,graph_2.motif_num
