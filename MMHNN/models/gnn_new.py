...
class GINet(nn.Module):
    def __init__(self, num_layer=5, emb_dim=300, feat_dim=256, dropout=0, pool='mean', device=torch.device("cuda:0")):
        ...
        self.feat_lin_2 = nn.Linear(600, self.feat_dim)
        ...
        # self.gib_module = GraphInformationBottleneckModule(device=self.device).to(self.device)
        # 定义 GIB 模块
        self.gib_module = GraphInformationBottleneckModule(
            device=self.device,
            node_input_dim=self.feat_dim,    # 设置输入维度为feat_dim (256)
            node_hidden_dim=self.emb_dim,    # 设置隐藏层维度为emb_dim (300)
            num_step_set2set=2  # 假设num_step_set2set为2，你可以根据实际需要调整
        ).to(self.device)

    def forward(self, data_1, data_2, gib=False):
        ...
        if gib:
            ...
            for idx_1, idx_2 in zip(data_1.motif_num, data_2.motif_num):
                # 获取图1的子图表示
                zitu_1 = h_sub_1[sum_1:sum_1 + idx_1, :]  # 将每个子图的特征作为新图的节点输入
                zitu_1_gib, KL_loss_1, _, _ = self.gib_module(zitu_1)  # 核心子图提取

                # 聚合操作，例如取均值或求和
                t1.append(zitu_1_gib.mean(dim=0, keepdim=True))

                # 获取图2的子图表示
                zitu_2 = h_sub_2[sum_2:sum_2 + idx_2, :]
                zitu_2_gib, KL_loss_2, _, _ = self.gib_module(zitu_2)  # 核心子图提取

                t2.append(zitu_2_gib.mean(dim=0, keepdim=True))
                KL_loss_sum += KL_loss_1 + KL_loss_2  # 累积 KL 损失
                sum_1 += idx_1
                sum_2 += idx_2
            # 将所有子图的特征拼接成一个批次
            t1 = torch.cat(t1, dim=0)  # (batch_size, dim)
            t2 = torch.cat(t2, dim=0)  # (batch_size, dim)
            # batch_size dim
            h_global_1 = self.feat_lin_2(t1)
            h_global_2 = self.feat_lin_2(t2)

            return h_global_1, h_global_2,KL_loss_sum
        else:
            h_global_1 = self.pool(h_1, data_1.batch)
            h_global_1 = self.feat_lin(h_global_1)
            h_global_2 = self.pool(h_2, data_2.batch)
    ...
    def get_subgraph(self, data_1, data_2, gib=False):
        data_1.x = self.initial_norm(data_1.x, data_1.batch)
        ...
        for idx_1, idx_2 in zip(data_1.motif_num, data_2.motif_num):
            # 获取图1的子图表示
            zitu_1 = h_sub_1[sum_1:sum_1 + idx_1, :]  # 将每个子图的特征作为新图的节点输入
            zitu_1_gib, KL_loss_1, preserve_rate_1, lambda_pos_1 = self.gib_module(zitu_1)  # 核心子图提取
            lambda_pos_1_list.append(lambda_pos_1)
            zitu_1_list.append(zitu_1)
            # 聚合操作，例如取均值或求和
            t1.append(zitu_1_gib.mean(dim=0, keepdim=True))

            # 获取图2的子图表示
            zitu_2 = h_sub_2[sum_2:sum_2 + idx_2, :]
            zitu_2_gib, KL_loss_2, preserve_rate_2, lambda_pos_2 = self.gib_module(zitu_2)  # 核心子图提取
            lambda_pos_2_list.append(lambda_pos_2)
            zitu_2_list.append(zitu_2)

            t2.append(zitu_2_gib.mean(dim=0, keepdim=True))
            KL_loss_sum += KL_loss_1 + KL_loss_2  # 累积 KL 损失
            sum_1 += idx_1
            sum_2 += idx_2
        return lambda_pos_1_list,lambda_pos_2_list,zitu_1_list,zitu_2_list,data_1.motif_batch,data_2.motif_batch
...
class GraphInformationBottleneckModule(nn.Module):
    ...
    def forward(self, features):
        # 如果 batch size 为1，直接返回nan
        if features.size(0) == 1:
            noisy_subgraphs = self.set2set(features)
            return noisy_subgraphs, 0, torch.tensor(1), torch.tensor(1)
        ...
        # 检查 NaN
        assert not torch.isnan(lambda_pos).any(), "NaN detected in lambda_pos"
        assert not torch.isnan(lambda_neg).any(), "NaN detected in lambda_neg"
        ...
        # 调试输出，检查数值范围
        # print('noisy_node_feature_mean min/max:', noisy_node_feature_mean.min(), noisy_node_feature_mean.max())
        # print('noisy_node_feature_std min/max:', noisy_node_feature_std.min(), noisy_node_feature_std.max())
        noisy_node_feature = noisy_node_feature_mean + torch.rand_like(noisy_node_feature_mean) * noisy_node_feature_std
        # 再次检查 NaN
        assert not torch.isnan(noisy_node_feature).any(), "NaN detected in noisy_node_feature"
        # print('noisy_node_feature.shape',noisy_node_feature.shape)
        noisy_subgraphs = self.set2set(noisy_node_feature)
        # print('noisy_subgraphs.shape',noisy_subgraphs.shape)
        # 检查 NaN
        assert not torch.isnan(noisy_subgraphs).any(), "NaN detected in noisy_subgraphs"
        # 计算 KL 损失
        epsilon = 1e-7
        ...
        # 检查 NaN
        assert not torch.isnan(KL_Loss).any(), "NaN detected in KL_Loss"
...
#         self.compressor = nn.Sequential(
#             nn.Linear(self.node_input_dim, self.node_hidden_dim),
#             nn.LayerNorm(self.node_hidden_dim),  # 使用 LayerNorm 替代 BatchNorm1d
#             nn.ReLU(),
#             nn.Linear(self.node_hidden_dim, 1)
#         )
#         self.num_step_set2set = num_step_set2set
...
#     def forward(self, features):
#         # 计算 lambda_pos 和 lambda_neg
#         # print('feautres size:',features.shape)
#         lambda_pos, p = self.compress(features)
#         lambda_pos = lambda_pos.reshape(-1, 1)
#         lambda_neg = 1 - lambda_pos
