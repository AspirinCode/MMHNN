...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        group_ids, res = getSubGraph(self.smiles_data[index][0])
        ...
        h=[data, group_ids, N,self.max_len]
        ...
        group_ids, res = getSubGraph(self.smiles_data[index][1])
        res = sorted(res, key=lambda t: len(t[1]), reverse=True)
        group_ids = sorted(group_ids, key=len, reverse=True)  # 将子图id按照子图大小 从大到小排列
        if (len(group_ids) > self.max_len):  # 将大于max_len的部分截去
            group_ids = group_ids[:self.max_len]
        ...
        data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr)
        t=[data, group_ids, N,self.max_len]
        return [h,t,self.smiles_data[index][2],self.smiles_data[index][3]]
...
def collate_fn(batch):
    ...
    print(r)
       '''
    datas, graphs_ids,atom_nums, max_len=zip(*h)
    ...
    data_1=datas
    datas, graphs_ids,atom_nums, max_len=zip(*t)
    datas = Batch.from_data_list(datas)  # 构建分子图batch
